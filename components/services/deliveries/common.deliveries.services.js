...
const findfilter = async (req) => {
  ...
  let totalResults = await Deliveries.find(query).populate({path: 'products',match: { weight: { $gte: req.body.weight } }}).countDocuments();
  ...
  let deliveries = await Deliveries.find(query).populate({path: 'products',match: { weight: { $gte: req.body.weight } }}).skip(skip).sort(sort).limit(limit);
